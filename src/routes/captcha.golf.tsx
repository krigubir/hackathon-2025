  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setWall1Y(prev => {
        ...
        if (next > 260) { next = 260; setWall1Dir(-1); }
        if (next < 60) { next = 60; setWall1Dir(1); }
        return next;
      });
      ...
      setWall2Y(prev => {
        ...
        if (next > 260) { next = 260; setWall2Dir(-1); }
        if (next < 60) { next = 60; setWall2Dir(1); }
        return next;
      });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setBall(prev => {
        ...
        // Smiley bounce
        const distToFace = Math.sqrt((newBall.x - smiley.x) ** 2 + (newBall.y - smiley.y) ** 2);
    ...
    >
      <div className="space-y-6">
        ...
        <div className="bg-border/30 rounded p-3 text-xs text-muted">
          <p>💡 Tip: Drag the ball in the opposite direction you want the ball to go. Longer drag = more power.</p>
        </div>
      </div>
